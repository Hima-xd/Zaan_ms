...
from KRISTY import dispatcher, telethn
from KRISTY.__main__ import (
    CHAT_SETTINGS,
    DATA_EXPORT,
    DATA_IMPORT,
    HELPABLE,
...
def load(update: Update, context: CallbackContext):
    ...
    try:
        imported_module = importlib.import_module("KRISTY.modules." + text)
    except:
...
def unload(update: Update, context: CallbackContext):
    ...
    unload_messasge.edit_text(
        f"ꜱᴜᴄᴄᴇꜱꜱꜰᴜʟʟʏ ᴜɴʟᴏᴀᴅᴇᴅ ᴍᴏᴅᴜʟᴇ : <b>{text}</b>",
        parse_mode=ParseMode.HTML,
    )
...
def listmodules(update: Update, context: CallbackContext):
    ...
    for helpable_module in HELPABLE:
        ...
        file_name = file_info.__name__.rsplit("KRISTY.modules.", 1)[1]
